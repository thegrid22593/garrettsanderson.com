<?php
# | p11creative mime email class
# | $Id: mime.php,v 1.1.1.1 2004/01/21 00:17:16 ryan Exp $

# | define a few MIME types
...
class MIME_mail {
  ...
  var $base64_func= '';      # | use default php base64 function
  var $qp_func = '';        # | leave blank for now
  var $mailer = "";          # | set to alternate mailer 
  ...
  function fattach($path, $description = "", $contenttype = OCTET, $encoding = BASE64, $disp = '') {
    $this->errstr = "";
    if (!file_exists($path)) {
      $this->errstr = printf("File '%s' does not exist", $path);
      return 0;
    }
  ...
  function build_message() {
    ...
    # | Attachment list is there.  Therefore MIME Message header must have multipart/mixed
